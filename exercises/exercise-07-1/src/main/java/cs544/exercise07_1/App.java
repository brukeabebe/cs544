       ...
       fillDataBase();
        ...
        try {
         ...
         // List<Flight> flights = session.createQuery("from Flight").list();
          ...
          Query query= session.createQuery("SELECT  f FROM Flight f JOIN f.airplane a where f.origin.country =:country and  a.capacity >500  ");
          query.setParameter("country","USA" );
            
            List<Flight> flightsA=query.list();
            
            System.out.println("Flight:  Departs:     "
                    + "                  Arrives:");
            for (Flight flight : flightsA) {
        ...
        try {
            ...
            Query query=session.createQuery("SELECT DISTINCT f.airline  from Flight f join f.airplane where f.airplane.model=:model");
        ...
        try {
            ...
            Query query=session.createQuery("SELECT  f FROM Flight f JOIN f.airplane ap JOIN f.airline air WHERE ap.model= :model AND air.name<>:star");
            query.setParameter("star", "Star Alliance").setParameter("model", "747");
            
            // TODO update HQL Star Alliance
           @SuppressWarnings("unchecked")
        ...
        try {
            ...
            Date d =new Date(2009,8, 7, 12, 0, 0);
            
           // System.out.println(d);
           
            //Query query= session.createQuery("SELECT f from Flight f WHERE f.departureTime <:time and f.departureDate<:date");
            //query.setParameter("time",d,TemporalType.TIME).setParameter("date", d,TemporalType.DATE );
            
           // Query query= session.createQuery("SELECT f from Flight f WHERE f.departureTime <:time and f.departureDate=:date");
           // query.setParameter("time","12:00:00 PM").setParameter("date", "2009-08-07");
            
            List<Flight> flights = session.createQuery("from Flight f where f.departureDate = '2009-08-07' and f.departureTime < '12:00:00 PM'").list();
